---
id: cx-automation
evdId: EVD--1
order: 1
org: Angelswing
period: "2022.09 ~ 2023.02"
title: "고객 문의 응대 및 자동화 구조 개선"
resultValue: "CX"
resultNum: null
resultUnit: null
skills:
  - Channel Talk
  - G Sheet
  - Slack
verified: 부분검증
capabilities: []
contentStatus: 완료
category: "Recruiting Ops / CRM"
coverImage: /portfolio/evd-1-cover.png
---

## resultLabel
CS/CX 담당자로서 문의를 직접 응대하며, 반복 문의 분류와 대응 기준을 정리해 응대 구조를 개선했습니다.

## headline
고객 문의를 직접 응대하며 반복 문의를 분류하고 자동화/가이드/챗봇 연계를 주도했습니다.

## summary
채널톡/전화 등 다양한 채널의 문의를 직접 응대하며 반복 문의를 분류하고 대응 기준을 정리했습니다. 자동화와 가이드 연계를 주도해 팀의 응대 부담을 낮추고 고객 경험이 일관되게 유지되도록 기여했습니다.

## problemDef
고객 문의가 많아지며 응대 지연과 팀 피로도가 커졌고, 고객 경험 개선이 필요했습니다.

## execution
문의 데이터를 유형화하고 난이도 기반 분기/자동응답/담당자 라우팅 체계를 설계해 주간 개선 루프를 운영했다.

## result
반복 문의를 줄이고 응대 품질을 안정화하는 데 기여했습니다. 해외 리서치/개발 팀과 협업 과정에서 한국 사업부를 대표해 커뮤니케이션을 수행했습니다.

## learning
자동화의 핵심은 답변 문구가 아니라 분기 기준의 정확도라는 점을 확인했습니다. 사람 개입 시점을 명확히 정의해야 고객 경험과 운영 효율을 동시에 지킬 수 있었습니다.

## approach
반복 문의를 분류하고 대응 기준을 정리해, 자동화/가이드/챗봇 연계를 주도했습니다. 주간 리뷰에서 태그 분포를 확인하며 챗봇 시나리오와 가이드 링크를 계속 보정했습니다.

## extensionView
반복 문의를 구조화한 방식은 이후 운영 과제에서도 재사용 가능한 기본 패턴이 되었습니다. 먼저 기준을 세우고 예외를 관리하는 접근을 유지하고 있습니다.

## candidConnection
고객 문의를 직접 응대하며 반복 문의를 분류하고 기준을 정리한 경험은, 후보자 상담에서도 고민을 정리하고 더 나은 안내를 제공하는 데 기여할 수 있습니다.
